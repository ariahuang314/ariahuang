...
def scoring_metric():
    ...
    JOIN esg_report AS er ON sd.company_name = er.firm_name
    WHERE sd.time = %s;
    """
    ...
    for index, row in company_score_data.iterrows():
        ...
        INSERT INTO esg_scores (
            report_id, report_year, company_name, Total_Score, Rating, E_Score, S_Score, G_Score, 
        ...
        cursor.execute(insert_query, (
            row['report_id'], row['report_year'], row['company_name'], row['Total_Score'], row['Rating'], row['E_Score'], row['S_Score'], row['G_Score'], 
